...
mt::ConditionVarDataWin32::ConditionVarDataWin32():
    mNumWaiters(0),
    ...
    mWasBroadcast(false)
{
    ...
    {
        throw mt::ThreadResourceException(
...
void mt::ConditionVarDataWin32::wait(HANDLE externalMutex)
{
    // Increment # waiting
    {
...
bool mt::ConditionVarDataWin32::wait(HANDLE externalMutex, double timeout)
{
    if (timeout == 0)
    {
        wait(externalMutex);
...
void mt::ConditionVarDataWin32::waitImpl(HANDLE externalMutex)
{
...
void mt::ConditionVarDataWin32::signal()
{
    bool haveWaiters;
    {
        const ScopedCriticalSection lock(mNumWaitersCS);
...
void mt::ConditionVarDataWin32::broadcast()
{
    bool haveWaiters;
    {
        const ScopedCriticalSection lock(mNumWaitersCS);
...
mt::ConditionVarWin32::ConditionVarWin32() :
    mMutexOwned(new mt::MutexWin32()),
    mMutex(mMutexOwned.get())
{}

mt::ConditionVarWin32::ConditionVarWin32(mt::MutexWin32 *theLock, bool isOwner) :
    mMutex(theLock)
{
    if (!theLock)
...
void mt::ConditionVarWin32::acquireLock()
{
    mMutex->lock();
}

void mt::ConditionVarWin32::dropLock()
{
    mMutex->unlock();
}

void mt::ConditionVarWin32::wait(double timeout)
{
...
void mt::ConditionVarWin32::signal()
{
    dbg_printf("Signalling condition\n");
    mNative.signal();
}

void mt::ConditionVarWin32::broadcast()
{
    dbg_printf("Broadcasting condition\n");
    mNative.broadcast();
}

mt::ConditionVarDataWin32& mt::ConditionVarWin32::getNative()
{
    return mNative;
}
