...
mt::ConditionVarIrix::ConditionVarIrix() :
    mMutexOwned(new mt::MutexIrix()),
    mMutex(mMutexOwned.get())
{
    dbg_ln("Creating a default condition variable");
}

mt::ConditionVarIrix::ConditionVarIrix(mt::MutexIrix *theLock, bool isOwner) :
    mMutex(theLock)
{
    if (!theLock)
...
void mt::ConditionVarIrix::acquireLock()
{
    mMutex->lock();
}

void mt::ConditionVarIrix::dropLock()
{
    mMutex->unlock();
}

void mt::ConditionVarIrix::signal()
{
...
void mt::ConditionVarIrix::wait()
{
    ...
    mt::ConditionVarIrix::wait(0);
}

void mt::ConditionVarIrix::wait(double timeout)
{
