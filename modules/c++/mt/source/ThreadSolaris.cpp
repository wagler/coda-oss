...
void mt::ThreadSolaris::start()
{
    ...
    if (p != 0)
    {
        ...
        throw mt::ThreadResourceException(message);
    }
}
...
void mt::ThreadSolaris::kill()
{
    dbg_printf("Killing thread\n");
    if ( thr_kill(mNative, SIGKILL) != 0)
    {
        throw mt::ThreadResourceException("thr_kill()");
    }
    setIsRunning(false);
}

void mt::ThreadSolaris::join()
{
    dbg_printf("Joining thread\n");
    if (::thr_join(mNative, NULL, NULL) != 0 )
        throw mt::ThreadResourceException("thr_join()");
}

void mt::ThreadSolaris::yield()
{
